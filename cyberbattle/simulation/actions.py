...
class Penalty:
    ...
    # penalty for generic suspiciousness通用可疑性的惩罚
    SUPSPICIOUSNESS = -5.0
...
class AgentActions:
    ...
    def __mark_node_as_discovered(self, node_id: model.NodeID):
        ...
        #return newly_discovered#zrf:本来无return
...
class DefenderAgentActions:
    ...
    def on_attacker_step_taken(self):
        ...
        for node_id, node_info in self._environment.nodes():
            ...
            for service in node_info.services:
                total_service_weights += service.sla_weight#Weight used to evaluate the cost of not running the service
    ...
    def on_attacker_step_taken_for_defender_reward(self):#zrf基于on_attacker_step_taken改变，为了给defender计算一次，来评估defender奖励的
        # Calculate the network availability metric based on machines
        # and services that are running
        total_node_weights = 0
        network_node_availability = 0
        ...
        network_availability = network_node_availability / total_node_weights
        assert(network_availability <= 1.0 and network_availability >= 0.0)
