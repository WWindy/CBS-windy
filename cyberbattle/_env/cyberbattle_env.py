...
from typing import NamedTuple, Optional, Tuple, List, Dict, TypeVar, TypedDict, cast,Any
...
import logging
import os
from cyberbattle.agents.baseline.multiagent.baseline_marlon_agent import BaselineAgentBuilder
from stable_baselines3 import PPO
from cyberbattle.agents.baseline.agent_wrapper_defender import DefenderEnvWrapper
from cyberbattle.agents.baseline.environment_event_source import EnvironmentEventSource
...
class CyberBattleEnv_chushi(gym.Env):
...
class CyberBattleEnv(gym.Env):
    ...
    def __init__(self,
        ...
        # number of entities in the environment network 静态defender
        self.__defender_agent=defender_agent
        ...
        #load_defender需要
        self.is_Learning_defender=False#为后续eval时调用训练好的defender
        if isinstance(defender_agent, LearningDefender1):  
            self.is_Learning_defender=True
            self.__defender_agent=None
            self.log_level = os.environ.get('LOGLEVEL', 'INFO').upper()
            self.logger = logging.Logger('marlon', level=self.log_level)
            self.console_handler = logging.StreamHandler()
            self.logger.addHandler(self.console_handler)
        
            self.event_source = EnvironmentEventSource()
            self.defender_builder=BaselineAgentBuilder(alg_type=PPO,policy='MultiInputPolicy')
            self.defender_wrapper=DefenderEnvWrapper(self,event_source=self.event_source)
            self.defender_agent=self.defender_builder.build(self.defender_wrapper,self.logger)
            self.defender_agent.setup_learn(total_timesteps=300000,eval_env=None,eval_freq=-1,n_eval_episodes=5,eval_log_path=None,reset_num_timesteps=True,tb_log_name="OnPolicyAlgorithm")
            self.defender_agent.on_rollout_start()#zrf加：为训练做准备？
    ...
    def __attacker_goal_reached(self) -> bool:
        ...
        if numpy.sum(self.episode_rewards) < goal.reward:
            return False
        ...
        if self.defender_agent is not None and \
                self._defender_actuator.network_availability >= goal.low_availability:
            return False
    ...
    def __defender_goal_reached(self) -> bool:
        """Check if defender's goal is reached(e.g. full eviction of attacker)"""
        goal = self.__defender_goal
        #goal.eviction默认为true，__get__owned_nodes_indices()函数作用是获取攻击者现在拥有节点的列表，该列表为空则防御者胜利
        #print(self.__get__owned_nodes_indices())
        return goal.eviction and not (self.__get__owned_nodes_indices())
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, StepInfo,bool, Any, Any,Any]:
        if self.__done:
            raise RuntimeError("new episode must be started with env.reset()")
        ...
        try:
            ...
            if self.__attacker_goal_reached() :#zrf:or self.__defender_constraints_broken():
                self.__done = True
                reward = self.__WINNING_REWARD
            elif self.__defender_goal_reached():
                #self.__done = True
            ...
            # Execute the defender step if provided
            d_continue,d_new_obs,d_dones,d_rewards=None,None,False,0
            if self.defender_agent:
                d_continue,d_new_obs,d_dones,d_rewards=self.defender_agent.perform_step(self.__stepcount)
                #a=self.defender_agent.wrapper.rewards
                #self._defender_actuator.on_attacker_step_taken()
                #self.defender_agent.step(self.__environment, self._defender_actuator, self.__stepcount)
                #self.defender_agent.executeAction()
                self.__owned_nodes_indices_cache = None
        ...
        return observation, reward, self.__done, info,d_continue,d_new_obs,d_dones,d_rewards
    ...
    def reset(self) -> Observation:
        ...
        self.__owned_nodes_indices_cache = None
        if self.defender_agent:
            self.defender_wrapper.timesteps=0
        return observation
    ...
    def render_as_fig(self):
        ...
        # plot the cumulative reward and network side by side using plotly
        fig = make_subplots(rows=1, cols=2)
        fig.add_trace(go.Scatter(y=numpy.array(self.episode_rewards).cumsum(),
                                 name='cumulative reward'), row=1, col=1)
        traces, layout = debug.network_as_plotly_traces(xref="x2", yref="y2")
        for t in traces:
            fig.add_trace(t, row=1, col=2)
