...
from cyberbattle.agents.baseline.multiagent.marlon_agent import MarlonAgent
from cyberbattle.agents.baseline.multiagent.multiagent_universe import AgentBuilder
# 所以一个应该是从一开始就训练。另一个是拿着已经下载的文件，然后再训练。 builder调用后面的数据。
class BaselineAgentBuilder(AgentBuilder):
    ...
    def __init__(self, alg_type: Type, policy: str):
        ...
        # 限制算法的类型必须是online的，然后输入policy，我认为这里的policy是具体的算法。
        self.alg_type = alg_type
        self.policy = policy

    def build(self, wrapper: GymEnv, logger: logging.Logger) -> MarlonAgent:
        model = self.alg_type(self.policy, Monitor(wrapper), verbose=1)#带进度条的输出日志信息
...
class LoadFileBaselineAgentBuilder(AgentBuilder):
    ...
    def build(self, wrapper: GymEnv, logger: logging.Logger) -> MarlonAgent:
        ...
        model.set_env(Monitor(wrapper))# 所以这里reset倒入的也是他自己的东西。
        return BaselineMarlonAgent(model, wrapper, logger)
# 主要是定义一些接口。
class BaselineMarlonAgent(MarlonAgent):
    ...
    def perform_step(self, n_steps: int) -> Tuple[bool, Any, Any]:
        ...
        # 格式问题。actoin应该是个动作编号。得到的最后格式是这样的[1,2,3,4,1,?]

        actions = actions.cpu().numpy()

        # Rescale and perform action 重新缩放
        clipped_actions = actions
        # Clip the actions to avoid out of bound error 对动作进行修剪，使动作不越界。让动作序号在最大值和最小值之间。
        ...
        #这里的step理解为操作。操作给定的动作，然后得到新的观察，奖励值等。
    ...
    def setup_learn(self,
        ...
        total_timesteps, callback = self.baseline_model._setup_learn(
            total_timesteps=total_timesteps,
            eval_env=eval_env,
            callback=None,#callback是一系列函数在训练程序中会被called在给定阶段，您可以在训练期间使用callback来访问 RL 模型的内部状态。它允许人们进行监控，自动保存，模型操控，进度条等
            eval_freq=eval_freq,
            n_eval_episodes=n_eval_episodes,
