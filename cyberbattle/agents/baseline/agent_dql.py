...
class DeepQLearnerPolicy(Learner):
    ...
    def optimize_model(self, norm_clipping=False):
        if len(self.memory) < self.batch_size:
            return
        #从记忆库中随机采样出一批样本，数量由batch_size指定。
        transitions = self.memory.sample(self.batch_size)
        # converts batch-array of Transitions to Transition of batch-arrays.将批次中的样本进行处理，转换为模型所需的数据格式。
        batch = Transition(*zip(*transitions))

        # Compute a mask of non-final states and concatenate the batch elements
        # (a final state would've been the one after which simulation ended)构建非最终状态的掩码及相关数据：
        ...
        #通过神经网络模型（policy_net）计算当前状态下各个动作的预期值（Q值）
        ...
        #计算下一状态的预期值（V值），用于更新Q值：next_state_values通过target_net计算下一状态的最优Q值，其中只考虑非最终状态，然后与奖励相加得到预期Q值。
        ...
        # Compute Huber loss
        #计算损失函数：使用Huber Loss（F.smooth_l1_loss）计算当前预期Q值和实际Q值之间的差距。
        loss = F.smooth_l1_loss(state_action_values, expected_state_action_values.unsqueeze(1))

        # Optimize the model
        #优化模型：将损失反向传播并更新模型参数，以使预期Q值逼近实际Q值。
    ...
    def save_model(self, filename):  
            # 保存模型的状态字典  
            torch.save(self.policy_net.state_dict(), f'{filename}_model.pth')  
              
            # 如果你想保存优化器的状态，可以这样做  
            # torch.save(self.optimizer.state_dict(), f'{filename}_optimizer.pth')  
              
            print(f'Model saved to {filename}_model.pth')  
            # 如果保存了优化器状态，则还会打印优化器保存的路径  
            # print(f'Optimizer state saved to {filename}_optimizer.pth')  
