...
class DefenderEnvWrapper(gym.Env, IEnvironmentObserver):
    ...
    def __init__(self,CyberBattleEnv,
        #cyber_env: cyberbattle_env.CyberBattleEnv,
        #attacker_reward_store: List[float] = [],
        event_source: Optional[EnvironmentEventSource] = None,
        defender: bool = False,
        max_timesteps=2048,#zrf!!
        invalid_action_reward=0,
        reset_on_constraint_broken = False):#zrf!!

        super().__init__()
        self.defender = None
        self.cyber_env=CyberBattleEnv
        self.bounds=self.cyber_env.bounds
        self.num_services = 0
        self.observation_space: Space = self.__create_observation_space()
        self.action_space: Space = self.__create_defender_action_space()
        ...
        #self.attacker_reward_store = self.cyber_env.episode_rewards#为什么要对attacker的奖励进行存储？        
        self.first = True
        self.reset_request = False#zrf加：这个是干嘛的？？
    ...
    def __create_observation_space(self) -> gym.Space:
        ...
        return spaces.Dict({'infected_nodes': spaces.MultiBinary(len(list(self.cyber_env.environment.network.nodes))),
                            'incoming_firewall_status': spaces.MultiBinary(len(self.firewall_rule_list)*len(list(self.cyber_env.environment.network.nodes))),
                            'outgoing_firewall_status': spaces.MultiBinary(len(self.firewall_rule_list)*len(list(self.cyber_env.environment.network.nodes))),
    ...
    def __create_defender_action_space(self) -> gym.Space:
        ...
        total_actions = 5
        reimage_node_number = len(self.cyber_env.environment.network.nodes)
        block_traffic_node = len(self.cyber_env.environment.network.nodes)
        block_traffic_port = 6
        block_traffic_incoming = 2
        allow_traffic_node = len(self.cyber_env.environment.network.nodes)
        allow_traffic_port = 6
        allow_traffic_incoming = 2
        stop_service_node = len(self.cyber_env.environment.network.nodes)
        stop_service_port = 3
        start_service_node = len(self.cyber_env.environment.network.nodes)
    ...
    def step(self, action):
        ...
        action_defender_reward=self.defender.executeAction(action,self.cyber_env.environment,self.cyber_env._defender_actuator)
        reward+=action_defender_reward
        ...
        if self.cyber_env.episode_rewards:
            reward += -1*self.cyber_env.episode_rewards[-1]
