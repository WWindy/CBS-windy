...
import sys
sys.path.append('/home/windy/Desktop/experiment/CyberBattleSim/MARLon-master')
#from cyberbattle.agents.baseline.env_wrappers.environment_event_source import EnvironmentEventSource

#from cyberbattle.agents.baseline.agent_wrapper import AttackerEnvWrapper
#from cyberbattle.agents.baseline.agent_wrapper import DefenderEnvWrapper
#from cyberbattle.agents.baseline.multiagent.evaluation_stats import EvalutionStats
from cyberbattle.agents.baseline.marlon_agent import MarlonAgent
#from cyberbattle.agents.baseline.multiagent import marl_algorithm


class AgentBuilder(ABC):
    '''Builder capable of creating a generic MarlonAgent given a wrapper.能够使用包装器创建通用 MarlonAgent 的构建器'''
    ...
    def build(self, wrapper: GymEnv, logger: logging.Logger) -> MarlonAgent:
        ...
        '''
        raise NotImplementedError #没有实现的错误。
...
class MultiAgentUniverse:
    ...
    def build(cls,
        ...
        defender_builder: Optional[AgentBuilder] = None,# 这里的optional指的是什么？可能是空
        defender_invalid_action_reward_modifier = -1,
        env_id: str = "CyberBattleToyCtf-v0",
        max_timesteps: int = 2000,            #防止每一次episode的训练时间过长
        attacker_loss_reward: float = -5000.0,#这里表示的是，攻击失败的奖励？
        defender_loss_reward: float = -5000.0,#表示防守失败的奖励？
        defender_maintain_sla: float = 0.60,
        defender_reset_on_constraint_broken: bool = True):# 这里的限制条件被破坏，指的是什么？，默认重置
        '''
        ...
        # SLA is an abbreviation for "Service Level Agreement". It means to have guaranteed reaction or resolving times for incidents (= trouble tickets).
            cyber_env = gym.make(
                ...
                # 防御方在构建对抗性隐藏欺骗网络的过程中必须尽量保持网络结构的一致性，而隐藏边和欺骗边的无限设置可能会极大地以一种不真实的方式改变网络结构，改变边的数量必须是有限的
